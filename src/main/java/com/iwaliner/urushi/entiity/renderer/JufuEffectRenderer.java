...
public class JufuEffectRenderer extends FallingBlockRenderer {
    ...
    public void render(JufuEffectDisplayEntity fallingBlockEntity, float f1, float f2, PoseStack poseStack, MultiBufferSource bufferSource, int i) {
        ...
        if (blockstate.getRenderShape() == RenderShape.MODEL) {
            ...
            if (blockstate != level.getBlockState(fallingBlockEntity.blockPosition()) && blockstate.getRenderShape() != net.minecraft.world.level.block.RenderShape.INVISIBLE) {
                poseStack.pushPose();
                ...
                for (var renderType : model.getRenderTypes(blockstate, RandomSource.create(blockstate.getSeed(fallingBlockEntity.getStartPos())), net.neoforged.neoforge.client.model.data.ModelData.EMPTY))
                    this.dispatcher.getModelRenderer().tesselateBlock(level, model, blockstate, blockpos.below(), poseStack, bufferSource.getBuffer(renderType), false, RandomSource.create(), blockstate.getSeed(fallingBlockEntity.getStartPos()), OverlayTexture.NO_OVERLAY, net.neoforged.neoforge.client.model.data.ModelData.EMPTY, renderType);
                poseStack.popPose();
